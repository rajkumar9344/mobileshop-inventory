        ...
        <div class="card-body">
            <form class="row align-items-end">
                <div class="col-lg-3 col-md-6 mb-2">
                    <label class="d-block mb-1">Supplier</label>
    ...
    @if(!empty($supplier_id))
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Supplier Ledger Report</h5>
                <div>
                    <a href="{{ route('reports.supplier-ledger-excel', ['supplier_id' => $supplier_id, 'start_date' => $start_date, 'end_date' => $end_date]) }}" target="_blank" class="btn btn-success btn-sm me-2"><i class="bi bi-file-earmark-excel me-1"></i>Export Excel</a>
                    <a href="{{ route('reports.supplier-ledger-pdf', ['supplier_id' => $supplier_id, 'start_date' => $start_date, 'end_date' => $end_date]) }}" target="_blank" class="btn btn-danger btn-sm me-2"><i class="bi bi-file-earmark-pdf me-1"></i>Export PDF</a>
                    <a href="{{ route('reports.supplier-ledger-print', ['supplier_id' => $supplier_id, 'start_date' => $start_date, 'end_date' => $end_date]) }}" target="_blank" class="btn btn-secondary btn-sm me-2"><i class="bi bi-printer me-1"></i>Print</a>
                    <form method="POST" action="{{ route('reports.supplier-ledger-send-email') }}" class="d-inline-block" style="display:inline;">
                        @csrf
            ...
            <div class="card-body p-0">
                        ...
                        <table class="table table-sm table-bordered mb-0">
                            ...
                            <tbody>
                                <tr class="table-secondary">
                                    ...
                                    <td class="text-right"><strong>{{ number_format($block['opening'],2) }}</strong></td>
                ...
                @empty
                    <div class="alert alert-info m-3">No transactions for selected supplier in this period.</div>
                @endforelse
            </div>
        </div>
    @endif
