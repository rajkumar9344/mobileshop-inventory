...
return new class extends Migration
{
    ...
    {
        // Add temporary BIGINT columns for monetary amounts in purchase_details
        Schema::table('purchase_details', function (Blueprint $table) {
            // Original table columns
            if (!Schema::hasColumn('purchase_details', 'price_temp')) {
                $table->bigInteger('price_temp')->nullable()->after('quantity');
            }
            if (!Schema::hasColumn('purchase_details', 'unit_price_temp')) {
                $table->bigInteger('unit_price_temp')->nullable()->after('price_temp');
            }
            if (!Schema::hasColumn('purchase_details', 'sub_total_temp')) {
                $table->bigInteger('sub_total_temp')->nullable()->after('unit_price_temp');
            }
            if (!Schema::hasColumn('purchase_details', 'product_discount_amount_temp')) {
                $table->bigInteger('product_discount_amount_temp')->nullable()->after('product_discount_type');
            }
            if (!Schema::hasColumn('purchase_details', 'product_tax_amount_temp')) {
            ...
            if (!Schema::hasColumn('purchase_details', 'amount_temp')) {
                $table->bigInteger('amount_temp')->nullable()->after('tax_amount_temp');
            }
        });

        // Copy data from old INTEGER columns to new BIGINT temp columns
        DB::statement("UPDATE purchase_details SET price_temp = price WHERE price IS NOT NULL");
        DB::statement("UPDATE purchase_details SET unit_price_temp = unit_price WHERE unit_price IS NOT NULL");
        DB::statement("UPDATE purchase_details SET sub_total_temp = sub_total WHERE sub_total IS NOT NULL");
        ...
        Schema::table('purchase_details', function (Blueprint $table) {
            if (Schema::hasColumn('purchase_details', 'price')) {
                $table->dropColumn('price');
            }
            if (Schema::hasColumn('purchase_details', 'unit_price')) {
                $table->dropColumn('unit_price');
            }
            if (Schema::hasColumn('purchase_details', 'sub_total')) {
                $table->dropColumn('sub_total');
            }
            if (Schema::hasColumn('purchase_details', 'product_discount_amount')) {
                $table->dropColumn('product_discount_amount');
            }
            if (Schema::hasColumn('purchase_details', 'product_tax_amount')) {
                $table->dropColumn('product_tax_amount');
            }
            if (Schema::hasColumn('purchase_details', 'mrp')) {
                $table->dropColumn('mrp');
            }
            if (Schema::hasColumn('purchase_details', 'cash_discount_amount')) {
                $table->dropColumn('cash_discount_amount');
            }
            if (Schema::hasColumn('purchase_details', 'rate')) {
                $table->dropColumn('rate');
            }
            if (Schema::hasColumn('purchase_details', 'tax_amount')) {
                $table->dropColumn('tax_amount');
            }
            if (Schema::hasColumn('purchase_details', 'amount')) {
    ...
    public function down(): void
    {
        ...
        DB::statement("UPDATE purchase_details SET price_temp = LEAST(price, 2147483647) WHERE price IS NOT NULL");
        DB::statement("UPDATE purchase_details SET unit_price_temp = LEAST(unit_price, 2147483647) WHERE unit_price IS NOT NULL");
        DB::statement("UPDATE purchase_details SET sub_total_temp = LEAST(sub_total, 2147483647) WHERE sub_total IS NOT NULL");
