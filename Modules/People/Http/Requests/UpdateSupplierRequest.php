...
class UpdateSupplierRequest extends FormRequest
{
    ...
    public function prepareForValidation()
    {
        ...
        // If 'type' is provided map it to legacy 'style' for storage
        if ($this->has('type')) {
            $this->merge(['style' => $this->input('type')]);
        }
    }
    ...
    public function rules()
    {
        ...
        return [
            ...
            'supplier_code'  => ['nullable','string','max:10', Rule::unique('suppliers','supplier_code')->ignore($supplierId)],
            'supplier_phone' => ['required','string','max:10','regex:/^[0-9]+$/', Rule::unique('suppliers','supplier_phone')->ignore($supplierId)],
            'supplier_email' => ['nullable','email','max:50','lowercase', Rule::unique('suppliers','supplier_email')->ignore($supplierId)],
            'area'           => 'nullable|string|max:30',
            'state'          => 'nullable|string|max:30',
            'city'           => 'nullable|string|max:30',
            'country'        => 'nullable|string|max:30',
            'address'        => 'nullable|string|max:200',
            'gst_no'         => ['nullable','string','max:15', Rule::unique('suppliers','gst_no')->ignore($supplierId)],
            'style'          => 'nullable|integer|in:1,2,3,4',
            'bank_name'      => 'nullable|string|max:50',
            'account_no'     => ['nullable','numeric', Rule::unique('suppliers')->where(function ($q) use ($ifsc) {
                if ($ifsc) {
