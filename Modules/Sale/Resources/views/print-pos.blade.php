        ...
        <table class="table-data">
            ...
            @foreach($sale->saleDetails as $saleDetail)
                <tr>
                    <td colspan="2">
                        ...
                        ({{ $saleDetail->quantity }} x {{ $pos_currency($saleDetail->price) }})
                    </td>
            ...
            @if($sale->tax_percentage)
                <tr>
                    ...
                    <th style="text-align:right">{{ $pos_currency($sale->tax_amount) }}</th>
                </tr>
            @endif
            @if($sale->discount_percentage)
                <tr>
                    <th colspan="2" style="text-align:left">Discount ({{ $sale->discount_percentage }}%)</th>
                    <th style="text-align:right">{{ $pos_currency($sale->discount_amount) }}</th>
                </tr>
            @endif
            @if($sale->shipping_amount)
                <tr>
                    <th colspan="2" style="text-align:left">Shipping</th>
                    <th style="text-align:right">{{ $pos_currency($sale->shipping_amount) }}</th>
                </tr>
            @endif
            <tr>
