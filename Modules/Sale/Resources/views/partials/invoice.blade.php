<div class="invoice-root" style="width:190mm; margin:0 auto; font-family: Arial, Helvetica, sans-serif; font-size:10px; color:#000; box-sizing:border-box;">
    ...
    {{-- ② TAX INVOICE title (center) + TRN (right corner) --}}
    <table style="width:100%; border-collapse:collapse; margin-bottom:10px;">
        <tr>
            <td style="width:30%;"></td>
            <td style="width:40%; text-align:center; vertical-align:middle; padding:4px 0;">
                <div style="font-size:17px; font-weight:900; letter-spacing:2px;">TAX INVOICE</div>
            </td>
    ...
    {{-- ③ BILL TO (left box) | gap | Invoice details (right box) — rounded separate boxes --}}
            ...
            <td style="width:49%; vertical-align:top; border:1px solid #ccc; border-radius:8px; padding:0;">
                <table style="width:100%; border-collapse:collapse; font-size:10px;">
                    <tr style="border-bottom:1px solid #eee;">
                        <td style="padding:8px 12px; font-weight:700; width:42%; color:#555;">Invoice No</td>
                        <td style="padding:8px 4px; width:5%; color:#999;">:</td>
                        <td style="padding:8px 12px; font-weight:700;">{{ $sale->reference }}</td>
                    </tr>
                    <tr>
                        <td style="padding:8px 12px; font-weight:700; color:#555;">Date</td>
                        <td style="padding:8px 4px; color:#999;">:</td>
                        <td style="padding:8px 12px;">{{ \Carbon\Carbon::parse($sale->date)->format('d-m-Y') }}</td>
                    </tr>
                </table>
            </td>
        </tr>
            ...
            <tr>
                ...
                <td style="width:45%; vertical-align:top; padding:0;">
                    <table style="width:100%; border-collapse:collapse;">
                        ...
                        <tr>
                            <td style="padding:7px 12px; font-weight:800; {{ $paid > 0 ? 'border-bottom:1px solid #eee;' : '' }}">NET TOTAL</td>
                            <td style="padding:7px 12px; text-align:right; font-weight:800; {{ $paid > 0 ? 'border-bottom:1px solid #eee;' : '' }}">{{ $currency($total_grand) }}</td>
                        </tr>
                        @if($paid > 0)
                        <tr>
                            <td style="padding:7px 12px;">PAID</td>
                            <td style="padding:7px 12px; text-align:right;">{{ $currency($paid) }}</td>
                        </tr>
                        @endif
                    </table>
                </td>
    ...
    @if($sale->note)
        <div style="margin-top:8px; font-size:10px;"><strong>Remarks:</strong> {{ $sale->note }}</div>
    @endif
