    ...
    <div class="card">
            ...
            <table class="banner-table">
                <tr>
                    <td class="company-details">
                        ...
                        <div class="subtitle">{{ settings()->company_tagline ?? 'Quotation from our company' }}</div>
                    </td>
        ...
        <div class="body">
            <div style="text-align:center; margin-bottom:8px;">
                ...
                <div style="font-weight:700">{{ \Carbon\Carbon::parse($quotation->date)->format('d/m/Y h:i A') }}</div>
            </div>
            ...
            <table class="cols-table">
                <tr>
                    ...
                    <td>
                        ...
                        <div class="muted">{{ $customer->customer_name ?? $quotation->customer_name }}</div>
                        <div class="muted">{{ $customer->address ?? $quotation->contact_address }}</div>
                        <div class="muted">Email: {{ $customer->customer_email ?? $quotation->contact_email }}</div>
                        <div class="muted">Phone: {{ $customer->customer_phone ?? $quotation->contact_phone }}</div>
                    </td>
                </tr>
            </table>

            <div class="message">
                <p style="margin:0 0 8px 0">Hello {{ $customer->customer_name ?? $quotation->customer_name }},</p>
                <p style="margin:0">Attached is your quotation. You can download the PDF attachment for your records.</p>
            </div>

            <div class="total">Total Amount: {{ format_currency($quotation->overall_net_rate ?: $quotation->overall_amount ?: $quotation->total_amount ?: 0) }}</div>
        </div>
